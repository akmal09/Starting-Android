...
class MainViewModel : ViewModel() {
    ...
    private val privateSnackbarText = MutableLiveData<Event<String>>()
    val snackbarText: LiveData<Event<String>> = privateSnackbarText
        ...
        client.enqueue(object : Callback<PostReviewResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    privateSnackbarText.value = Event(response.body()?.message.toString())
                }else{
                    Log.e(TAG, "onFailure: ${response.message()}")
                }
            }
